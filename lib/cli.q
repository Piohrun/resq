...
.tst.parseCLI:{[args]
    ...
    invalidRanges: where
        ...
        ((numericOptionNames in `flakeEvidenceMin`benchmarkMinSamples) and numericValues < 2) or
        ((numericOptionNames = `benchmarkAlphaPercent) and numericValues = 0) or
    ...
    if[count invalidRanges;
        ...
        requirement: $[numericOptionNames[badIndex] in `isolateTimeout`isolateWorkers`shardCount`coverageContextMax`coverageContextEntryMax`flakeFailureMin`flakeWindow; "> 0";
                       numericOptionNames[badIndex] in `flakeEvidenceMin`benchmarkMinSamples; ">= 2";
                       numericOptionNames[badIndex]~`benchmarkAlphaPercent; "between 1 and 100";
    ...
    reportProfile:$[0=count options`reportProfile;
        lower .tst.toString @[get;`.tst.app.reportProfile;`full];
        lower options`reportProfile];
    if[not reportProfile in ("full";"results";"telemetry");
        :.tst.cli.error "report-profile must be one of: full, results, telemetry"];
    if[(shardCount>1) and not ("full"~reportProfile);
        :.tst.cli.error "sharded runs require report-profile full"];
    if[0<count options`labels;
        parsedLabels:.tst.parseLabelJson options`labels;
        if[not parsedLabels`ok;
            :.tst.cli.error "Invalid labels: ",parsedLabels`error];
        options[`labels]:parsedLabels`labels];
...
printUsage:{[]
    {-1 x;} each (
        ...
        "  -snapshot-audit       Write a complete/partial snapshot inventory";
        "  -snapshot-gate        Require complete inventory with no missing/obsolete snapshots";
        "  -benchmark-baseline FILE  Compare perf samples with a versioned baseline";
        "  -benchmark-gate       Fail on significant, practically meaningful regressions";
        ...
        "  -labels JSON          Add bounded string labels (CLI overrides env/config)";
        "  -no-vcs               Disable repository context discovery";
        "";
        "REPORTS";
        "  -junit | -xunit | -json      Write a report to -outDir (default '.')";
        "  -report-profile full|results|telemetry  JSON evidence profile (default full)";
        "                        Sharding and release qualification require full";
        "  -final-diffs [-final-diff-limit N]  Repeat bounded diffs in the final report";
        ...
        "  -cov-completeness-min N  Gate statement instrumentation completeness";
        "  -cov-branches-min N   Gate branch-edge coverage; partial data fails closed";
        "  -cov-branch-completeness-min N  Gate branch-site instrumentation";
        "  -cov-allow-partial    Allow a line gate to use a partial denominator";
...
initCLI:{[parsed]
    ...
    if[options`snapshotAudit;.tst.app.snapshotAudit:1b];
    if[options`snapshotGate;.tst.app.snapshotGate:1b;.tst.app.snapshotAudit:1b];
    if[options`benchmarkGate;.tst.app.benchmarkGate:1b;.tst.app.runPerformance:1b];
    if[options`benchmarkAcceptEnvironment;.tst.app.benchmarkAcceptEnvironment:1b];
    ...
    if[0<count options`reportProfile;.tst.app.reportProfile:`$lower options`reportProfile];
    if[options`noVcs;.tst.app.vcsProbe:0b];
    if[options`finalDiffs;.tst.app.finalDiffs:1b];
    if[not 10h=type options`finalDiffLimit;
        .tst.app.finalDiffLimit:"j"$options`finalDiffLimit];
    if[99h=type options`labels;
        .tst.app.labels:.tst.mergeLabels[.tst.app.labels;options`labels]];
    ...
    if[0<count options`benchmarkBaseline;
        .tst.app.benchmarkBaseline:options`benchmarkBaseline;
        .tst.app.runPerformance:1b];
    if[not 10h=type options`benchmarkAlphaPercent;
        .tst.app.benchmarkAlphaPercent:"j"$options`benchmarkAlphaPercent];
    if[not 10h=type options`benchmarkEffectMin;
        .tst.app.benchmarkEffectMin:"j"$options`benchmarkEffectMin];
    if[not 10h=type options`benchmarkMinSamples;
        .tst.app.benchmarkMinSamples:"j"$options`benchmarkMinSamples];
    ...
    if[not 10h=type options`coverageFunctionMin;
        .tst.app.coverageFunctionMin:options`coverageFunctionMin;
        .tst.app.runCoverage:1b];
    if[not 10h=type options`coverageLineMin;
        .tst.app.coverageLineMin:options`coverageLineMin;
        .tst.coverageStatements:1b;
        .tst.app.runCoverage:1b];
    if[not 10h=type options`coverageCompletenessMin;
        .tst.app.coverageCompletenessMin:options`coverageCompletenessMin;
        .tst.coverageStatements:1b;
        .tst.app.runCoverage:1b];
    if[not 10h=type options`coverageBranchMin;
        .tst.app.coverageBranchMin:options`coverageBranchMin;
        .tst.coverageBranches:1b;
        .tst.app.runCoverage:1b];
    if[not 10h=type options`coverageBranchCompletenessMin;
