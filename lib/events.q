...
.tst.lifecycleEvents:{[runModel;manifest]
    ...
    rows:.tst.resultRows runModel;
    rowPositionsByFile:group `$({.tst.toString x`file} each rows);
    events:();
    sequence:1j;
    ...
    while[fi<count selectedFiles;
        ...
        fileRows:rows $[(`$filePath) in key rowPositionsByFile;
            rowPositionsByFile (`$filePath);`long$()];
        ...
        sequence+:1;
        rowPositionsBySuite:group `$({.tst.toString x`suite} each fileRows);
        suites:string key rowPositionsBySuite;
        si:0;
        while[si<count suites;
            suiteName:suites si;
            suiteId:.tst.manifestSuiteId[filePath;suiteName];
            suiteRows:fileRows rowPositionsBySuite (`$suiteName);
