...
### Fixed
- **Dependency analysis is now lexical, bounded, and transactional.** Executable `\l` and `.utl.require` directives are distinguished from strings, comments, and terminated script tails; direct absolute targets are no longer confused with concatenated package suffixes. Relative native loads resolve from CWD and then QHOME against an indexed, case-safe discovery set without interning rejected filesystem probes. Bounded regular-file snapshots, file/source/directive/edge/path ceilings, exact rescans, registry validation, preflighted batch path admission, atomic graph publication, and bounded iterative traversal prevent stale edges, partial rebuilds, resource exhaustion, unpublished target-symbol leaks, and recursive cycle failures.
- **Reporter serialization now has bounded admission and assembly.** Direct result rows are canonicalized before output, every retained text/list field honors the configured diagnostic ceilings, aggregate expansion is rejected before rendering, and a hard 100,000-row ceiling prevents unbounded report work. JSON is serialized one admitted row at a time, JUnit/xUnit/text group suites in one pass, and fixed-size line chunks enforce the 32 MiB artifact ceiling without unbounded whole-payload or quadratic assembly.
- **File reporters now retain format and publication authority.** Repeated JUnit/xUnit/JSON/text initialization cannot reuse a stale cached builder, the selected XML builder is fixed for the current run, and missing or malformed module exports fail closed. JSON and XML artifacts are byte-bounded, reject unsafe target types and malformed paths, and use the same atomic same-directory replacement path as coverage; serialization or rename failures preserve the previous valid report and fail the run.
- **Coverage accounting and publication are now fail-closed.** Single-file runs no longer collapse the nested counter registry into an unextendable keyed table; explicit reloads preserve owned wrappers and reject malformed lifecycle state; failed initialization/load instrumentation unwinds cleanly; counters, files, functions, source snapshots, and report sizes are bounded and validated; HTML fields are escaped; and each report file is replaced atomically without following an existing symlink.
...
- `getDependents` now uses bounded iterative traversal, so deep or circular `\l`/`require` graphs cannot overflow the q stack.
