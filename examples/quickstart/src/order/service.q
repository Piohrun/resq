...
system "d .order";
  ...
  / Order database
  .order.orders: ([id:`long$()] userId:`long$(); items:(); totalAmount:`float$(); status:`symbol$(); createdAt:`timestamp$());
  ...
  create:{[userId;items]
      ...
      / Generate order ID
      newId: $[0<count .order.orders; 1 + max (key .order.orders)`id; 1];
  ...
  paymentCallback:{[orderId;status;txnId]
      if[not orderId in (key .order.orders)`id; :()];
  ...
  / Get order by ID
  findById:{[id]
      $[id in (key .order.orders)`id; .order.orders[id]; ()]
   };

  / Get user orders
  getByUser:{[userId]
      select from .order.orders where userId=userId
   };
